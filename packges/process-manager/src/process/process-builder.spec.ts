...
describe('toshokan-book-manager', () => {
  describe('task-manager', () => {
    describe('process-builder', () => {
      ...
      type Payload = number;
      const processName: ProcessName = { name: 'to-do-process', version: '1.0' };
      ...
      it('add steps', () => {
        ...
        const closedStep = process.graph.searchNodes((n) => n.status === 'closed')[0];
      ...
      it('add relation', () => {
        ...
        const closeEdge = process.graph.searchEdges((n) => n.command === 'close')[0];
