...
export function addActions<S extends string, P, C extends string>(
  ...
  return (context) => {
    actions.forEach(([command, action]) => context.setInstance(command, action));
    return context;
  };
}
...
export function addRelations<S extends string, P, C extends string>(
  ...
  return (process) => {
    relations.forEach(([from, to, weight]) => process.addRelation(from, to, weight));
    return process;
  };
}
...
export class ProcessBuilder<S extends string, P, C extends string> {
  ...
  pipe(...operators: ProcessBuilderOperators<S, P, C>[]): ProcessBuilder<S, P, C> {
    this.operators = this.operators.concat(operators);
    return this;
  }

  build<Process extends IProcess<S, P, C>>(factory: ProcessFactory<S, P, C>): Process {
